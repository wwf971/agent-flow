...
| `metadata` | `jsonb` | flexible metadata object, must include `evetList` |
...
The backend should always normalize `metadata.evetList` as a string array before writing.
...
Event order is not stored in the event row. Event order is stored in `conversation.metadata.evetList`.
...
PostgreSQL delivers notifications after transaction commit. If multiple events are inserted in one transaction, clients receive them only after that transaction finishes.
